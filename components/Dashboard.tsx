...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-[#08080f] text-white">
      ...
      <header className="sticky top-0 z-10 border-b border-[#1e1e2e] bg-[#08080f]/90 backdrop-blur-sm">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="h-7 w-7 rounded-md bg-[#00fff9] flex items-center justify-center">
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                <path d="M3 3h7v7H3V3zm11 0h7v7h-7V3zM3 14h7v7H3v-7zm11 0h7v7h-7v-7z" fill="#08080f" strokeWidth="0"/>
              </svg>
            </div>
            <div>
      ...
      {/* Body */}
      <main className="mx-auto max-w-[1600px] px-6 py-6 flex flex-col gap-6">
        {error && (
        ...
        {data && (
          <>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              />
              <SummaryChip
                label="Total Conversions Today"
                value={data.accounts
                  .reduce((s, a) => s + a.metrics.today.conversions, 0)
                  .toLocaleString()}
                accent
              />
              <SummaryChip
                ...
                value={`$${data.accounts
                  .reduce((s, a) => s + a.metrics.today.spend, 0)
                  .toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
              />
              <SummaryChip
                label="Total Clicks Today"
                value={data.accounts
                  .reduce((s, a) => s + a.metrics.today.clicks, 0)
                  .toLocaleString()}
              />
            </div>
...
function SummaryChip({
  ...
  return (
    <div className="rounded-lg border border-[#1e1e2e] bg-[#111118] px-4 py-2.5 flex items-center gap-3">
      ...
      <p
        className={`text-sm font-bold ${accent ? "text-[#00fff9]" : "text-white"}`}
      >
        {value}
      </p>
    </div>
