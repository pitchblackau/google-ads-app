...
export function generateSuggestions(data: SuggestionsRawData): Suggestion[] {
  ...
  // statusCode 5 = NONE — triggered but not yet explicitly added
  ...
  if (singleAdGroups.length > 0) {
    const examples = singleAdGroups
      .slice(0, 2)
      .map((ag) => `"${ag.adGroupName}"`)
      .join(", ");
    out.push({
      id: nextId(),
      type: "opportunity",
